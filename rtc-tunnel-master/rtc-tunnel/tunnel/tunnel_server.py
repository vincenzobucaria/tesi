...
from aiortc import RTCSessionDescription, RTCPeerConnection, RTCDataChannel, RTCConfiguration, RTCIceServer
...
class TunnelServer:
    def __init__(self, stun_server_ip_reg, signal_server):
        self._signal_server = signal_server
        
        pattern = r'http://[^/]+@([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)'
        match = re.search(pattern, stun_server_ip_reg)
        
        global stun_server_ip 
        stun_server_ip = match.group(1)
        print(stun_server_ip)
        #print("IP STUN SERVER: ", stun_server_ip)
        self._tasks = Tasks()
    ...
    async def _handle_new_client_async(self, obj: RTCSessionDescription, src: str):
        ...
        peer_connection = RTCPeerConnection(configuration=RTCConfiguration([RTCIceServer("stun:"+stun_server_ip+":19302")]))
        ...
        print(peer_connection.localDescription, src)
    ...
    def _configure_healthcheck(self, channel: RTCDataChannel, peer_connection: RTCPeerConnection(configuration=RTCConfiguration([RTCIceServer(stun_server_ip+":19302")]))):
