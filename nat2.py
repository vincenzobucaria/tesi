...
class custom_network(Topo):
    ...
    def build(self, n=2):
        ...
        for h in range(n):
            host = self.addHost('x%s' % h, ip="80.0.0.{}".format(x), defaultRoute='via 80.0.0.1')
            self.addLink(host, switch2)
            x = x+1


        r2 = self.addNode('r2', cls=LinuxRouter, ip=router2_internal_ip)
        self.addLink(r2, switch2, intfName1='r2-eth0', params1={'ip': router2_internal_ip+"/24"})
