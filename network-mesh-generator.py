...
def network_topology_creator(net): #Funzione che permette di istanziare la topologia
    ...
    def build_public_lan(lan, x, y): #Genera la rete pubblica relativa a una LAN (passata come parametro)
            default_x = x
            mask, subnet, end_ip = subnet_calculator(default_x, len(public_network_topology[lan])) #E' necessario determinare la maschera di sottorete per garantire un routing corretto
    ...
    x = host_creator(x) #Per prima cosa crea le LAN private


    #Successivamente crea le LAN con indirizzi pubblici 
    ...
    for lan in private_network_topology:
        ...
        nat_routers[lan] = net.addHost("r{}".format(x),cls=LinuxRouter, ip=nat_private_internal_ip+"/24") #Router che applica CG-NAT
        ...
        nat_routers[lan].cmd("ip route add ", border_router_internal_ip[:-3], " dev r{}-eth1".format(x))
    ...
    topologyProperties("properties.txt") #Chiama la funzione che legge i parametri desiderati per gli archi e crea le relative strutture dati
...
def server_setup(net):
    for node in servers:
        server = net.getNodeByName(node)
        
    
        server.cmd("cd rtc-signal-server-master && gradle bootRun &")
...
if __name__ == '__main__':
    ...
    print("*** Il server pubblico è ------> ", servers[0], ", RICORDATI DI AVVIARE LO STUN SERVER ***\n\n")

    print("*** Sto configurando il server ***\n\n")
    server_setup(net)
